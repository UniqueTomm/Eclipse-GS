...
namespace FortWeapon
{
	...
	void hkHandleReloadCost(AFortWeapon* Weapon, int AmountToTake)
	{
		...
		EFortWeaponReloadType ReloadType = LootManager::GetReloadType(reinterpret_cast<UFortWeaponItemDefinition*>(FoundItemEntry->ItemDefinition));

		if (ClipSize > 0)
		{
			if (ReloadType == EFortWeaponReloadType::ReloadWholeClip)
			{
				AmountToTake = FoundAmmoItemEntry->Count >= ClipSize - FoundItemEntry->LoadedAmmo ? ClipSize - FoundItemEntry->LoadedAmmo : FoundAmmoItemEntry->Count;

				FoundItemEntry->LoadedAmmo += AmountToTake;
				Weapon->AmmoCount = FoundItemEntry->LoadedAmmo;
			...
			else if (ReloadType == EFortWeaponReloadType::ReloadIndividualBullets)
			{
				FoundItemEntry->LoadedAmmo += AmountToTake;
				Weapon->AmmoCount = FoundItemEntry->LoadedAmmo;
				PlayerController->WorldInventory->Inventory.MarkItemDirty(*FoundItemEntry);
